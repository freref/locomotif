...
class LoCo:
    ...
    def find_best_pair(self, l_min=None, l_max=None, vwidth=None):
        ...
        path = loco_jit.find_best_pair_with_bp(csm, endpoint_csm, mask, self.warping, bp_dir)
        if len(path) == 0:
            return None
...
def find_best_pair(csm, endpoint_csm, mask, warping=True, bp_dir=None):
    return loco_jit.find_best_pair_with_bp(csm, endpoint_csm, mask, warping, bp_dir)
