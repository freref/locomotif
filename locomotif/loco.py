...
class LoCo:
    ...
    def calculate_cumulative_similarity_matrix(self):
        ...
        if self.warping:
            self._csm, self._span_i, self._span_j = loco_jit.cumulative_similarity_matrix_warping_with_spans(
                self._sm, self.tau, self.delta_a, self.delta_m, self._symmetric, 0
            )
        else:
            self._csm, self._span_i, self._span_j = loco_jit.cumulative_similarity_matrix_no_warping_with_spans(
                self._sm, self.tau, self.delta_a, self.delta_m, self._symmetric, 0
            )
        return self._csm
    ...
    def find_best_pair(self, l_min=None, vwidth=None):
        ...
        if self.warping:
            csm, endpoint_csm = loco_jit.cumulative_similarity_matrix_warping_for_pair(
                self._sm, self.tau, self.delta_a, self.delta_m, self._symmetric, diag_offset, min_path_length
            )
        else:
            csm, endpoint_csm = loco_jit.cumulative_similarity_matrix_no_warping_for_pair(
                self._sm, self.tau, self.delta_a, self.delta_m, self._symmetric, diag_offset, min_path_length
            )

        mask = np.full(csm.shape, False)
        path = loco_jit.find_best_pair(csm, endpoint_csm, mask, self.warping)
        if len(path) == 0:
            return None
